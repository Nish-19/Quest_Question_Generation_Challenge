...
with AWP strategy
refer to awp.py
https://github.com/antmachineintelligence/Feedback_1st/blob/a8d201a7d91e967f3df508434f7969ca9f59d0d6/utils/utils.py#L622
...
os.environ['WANDB_NOTEBOOK_NAME'] = 'FinetuneT5_awp'
...
def construct_task(answer, question_type):
    # TODO: try variations: the description from the compeittion website
    # NOTE: be careful about tokenization
    ...
    if question_type == 'action':
        ...
        task = f'Based on the story, write a question that asks test takers to identify the character of the story, "{answer}," or describe the characteristics of this character. The question usually starts with "Who," "Whose," "What," or "How."'
        # The character "{answer}" cannot appear in your question.'
    elif question_type == 'prediction':
        task = f'Based on the story, write a question that asks for a logical consequence. The question usually has the template "What/How/Who will... after/when/if... ?" or "What/How/Who will happen if... ?" The answer to the question is "{answer}".'
        # The question cannot contain the phrase "{answer}".'
    elif question_type == 'outcome resolution': # Need some thoughts on this
        task = f'Based on the story, write a question that asks to identify the outcome "{answer}" caused by something in the story. The question is usually worded as "What happened/happens/has happened. . . after...".'
        # The question cannot contain the phrase "{answer}".'
    elif question_type == 'causal relationship':
        task = f'Based on the story, write a question that asks about an effect caused by "{answer}". The question usually begins with "Why" or "What made/makes".'
        # The question cannot contain the phrase "{answer}".'
    else:
...
# Tokenization
def get_t5_encoding(model_name, t5_inputs, question, mask_task=False):
    tokenizer = T5Tokenizer.from_pretrained(model_name)
    ...
    # optionally mask the task
    if mask_task:
        for idx in range(labels.shape[0]):
            # NOTE: the task begin and end token indices are hardcoded
            task_begin_idx = torch.where(labels[idx] == tokenizer.convert_tokens_to_ids(']'))[0][1] + 1
            try:
                task_end_idx = torch.where(labels[idx] == tokenizer('[').input_ids[0])[0][2]
            except:
                print(tokenizer.decode(labels[idx]))
                raise Exception('Error masking task')
            labels[idx, task_begin_idx:task_end_idx] = -100
...
class FinetuneT5(pl.LightningModule):
    def __init__(self, model_name, niters=None, lr=3e-4, num_train_epochs=5, warmup_steps=1000, args=None):
        '''
        args is specifically for AWP
        '''
        super().__init__()
        self.automatic_optimization = False # Invokes manual optimization        
        ...
        self.scheduler = opt_configs['lr_scheduler']
        ...
        self.awp = AWP(self.model,
          ...
          start_epoch=0, #args.num_train_steps/args.epochs,
          scaler=self.scaler
        )
    ...
    def training_step(self, batch, batch_idx):
        '''
        Compared to t5_finetune.py, this function will additionally handle optimization
        '''
        ...
        self.awp.attack_backward(input_ids,labels,attention_mask,1) # hardcode to 1 so that AWP always start at beginning
...
def add_params():
    ...
    parser.add_argument("-S", "--strategy", type=str, default='ddp',
                        help='distributed strategy. {ddp, deepspeed}')
    ...
    parser.add_argument("--mask_task", type=bool, default=False,
                        help="Whether to mask out the task tokens during training. Use only with 'story-taskAsInput'")
    ## AWP params
...
if __name__ == '__main__':
    args = add_params()
    run_name  = f'AWP.advlr-{args.adv_lr}.adveps-{args.adv_eps}.{args.model_name}.IOType-{args.io_type}.Qprefix-{args.question_prefix}.ep-{args.num_epochs}.bs-{args.batch_size}.lr-{args.learning_rate}.Ndevices-{args.num_devices}.strategy-{args.strategy}.maskTask-{args.mask_task}'
    ...
    train_input_ids, train_attention_mask, train_labels = get_t5_encoding(args.model_name, train_inps, train_question, mask_task=args.mask_task)
    val_input_ids, val_attention_mask, val_labels = get_t5_encoding(args.model_name, val_inps, val_question, mask_task=args.mask_task)
    ...
    if args.strategy == 'ddp':
        ...
        raise Exception('unknown distributed strategy, must be {ddp, deepspeed}')
