...
def get_parallel_corpus(df):
    # hash stories and sections
    story, answer, question = [], [], []
    for i, row in df.iterrows():
        story.append(row['content'])
...
# Constrcut transformer input 
def construct_transformer_input_old_vary(story, question, choice=1):
    inps = []
    if choice == 1:
        prefix = 'Generate answer from context and question: '
        suffix = ''
    elif choice == 2:
    ...
    for stry, ques in zip(story, question):
        transformer_input = prefix + '\nContext: ' + stry + '\nQuestion: ' + ques + suffix
        inps.append(transformer_input)
    return inps


# Constrcut transformer input 
def construct_transformer_input_bart(story, question):
    inps = []
    for stry, ques in zip(story, question):
...
def remove_duplicates(df):
    clean_df = df.drop_duplicates(subset='generated_question')
    return clean_df
...
def add_params():
    ...
    parser.add_argument("-EFN", "--eval_filename", type=str, default="nucleus_flan_t5_large_org_0.95_1.20_40.csv", help="Evaluation filename")
    parser.add_argument("-F", "--eval_folder", type=str, default="results_org", help="Evaluation Folder where output is saved (testset for testing on test set)")
...
if __name__=='__main__':
    ...
    test_df = remove_duplicates(test_df_raw)
    print('Original Length of data: {:d}'.format(len(test_df_raw)))
    print('Length of data after removing duplicates: {:d}'.format(len(test_df)))

    test_story, test_answer, test_question = get_parallel_corpus(test_df)


    # %%
    test_inps = construct_transformer_input_old_vary(test_story, test_question, args.prefix_choice)
    ...
    # %%
    test_input_ids, test_attention_mask, test_labels = get_transformer_encoding(tokenizer, test_inps, test_answer)
    ...
    search_dir = os.path.join('./code/ans_generation', args.checkpoint_folder, args.run_name)
    ...
    if args.decoding_strategy == 'N':
        times = [args.num_of_samples for _ in range(len(test_df))]
        new_val_df = test_df.loc[test_df.index.repeat(times)].reset_index(drop=True)
    ...
    new_val_df['generated_answer'] = val_preds

    output_path = os.path.join(RAW_DIR, "results_org_ans")
