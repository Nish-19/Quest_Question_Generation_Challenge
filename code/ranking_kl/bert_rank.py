...
def get_dataloader(batch_size, tokenize_dict, rouge_scores):
...
class RougeKLModel(nn.Module):
    ...
    def forward(self, input_ids, token_ids, attn_masks):
...
def train(use_wandb, epochs, model, device, train_dataloader, val_dataloader, optimizer, scheduler, alpha1, alpha2, save_path):
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            # Forward pass
            scores = model(b_inpids, b_ttids, b_attnids)
        ...
        for val_step, val_batch in tqdm(enumerate(val_dataloader)):
            ...
            with torch.no_grad():
                valscores = model(b_inpids_val, b_ttids_val, b_attnids_val)
                # NOTE: KL Loss Calculation
...
def main():
    ...
    # get input
    train_context_ans, train_question = get_transformer_input(train_df)
    val_context_ans, val_question = get_transformer_input(val_df)
    ...
    train_dataloader = get_dataloader(batch_size, train_tokenized_dict, train_df['Rouge_L'])
    val_dataloader = get_dataloader(batch_size, val_tokenized_dict, val_df['Rouge_L'])
    ...
    # Initialize model
    model = RougeKLModel(model_name='bert-base-uncased', hid_dim=args.hidden_dim).to(device)    
    ...
    train(args.wandb, args.num_epochs, model, device, train_dataloader, val_dataloader, optimizer, scheduler, args.alpha1, args.alpha2, save_path)
