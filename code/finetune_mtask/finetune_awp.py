...
def construct_transformer_output(question, attribute):
    ops = []
    for quest, attr in zip(question, attribute):
        transformer_output = 'The attribute is: {:s}\n The question is: {:s}'.format(attr, quest)
...
def get_transformer_encoding(tokenizer, transformer_inputs, transformer_outputs):
    # tokenizer = T5Tokenizer.from_pretrained(model_name)
    max_source_length, max_target_length = 1024, 256
...
if __name__ == '__main__':
    ...
    train_ops = construct_transformer_output(train_question, train_attr)
    val_ops = construct_transformer_output(val_question, val_attr)
    ...
    save_name = 'mtask_' + args.run_name 
